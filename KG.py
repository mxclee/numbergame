...
from streamlit_graph import agraph, Node, Edge, Config
import pandas as pd
...
df_disease = pd.DataFram(final_arr_short.neighbour.name.value_counts().reset_index().values, columns=["name", "count"])
...
df_connections = df_connections[df_connections.neighbor_name != 'na']
